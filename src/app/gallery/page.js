...
export default function GalleryPage() {
  ...
  return (
    <div className="min-h-screen bg-[#071322] text-white flex flex-col selection:bg-[#C6A98A] selection:text-black">
      <Navbar />
      ...
      <main className="flex-grow pt-28 pb-20">
        ...
        <section className="relative px-4 sm:px-6 lg:px-8 py-12 max-w-7xl mx-auto text-center space-y-5 overflow-hidden">
          <div className="animate-breathe absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[300px] bg-[#C6A98A]/10 blur-[140px] pointer-events-none rounded-full" />
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-5 py-2 text-xs sm:text-sm font-semibold uppercase tracking-wider rounded-none transition-all duration-300 cursor-pointer ${
                    isActive
                      ? "bg-[#C6A98A] text-black shadow-lg scale-105 font-bold"
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-5 sm:gap-6">
              {filteredItems.map((item, idx) => (
                <Reveal
                  ...
                  className="group relative block w-full mb-5 sm:mb-6 break-inside-avoid overflow-hidden bg-zinc-950 border border-zinc-800/80 hover:border-[#C6A98A]/60 transition-colors duration-500 cursor-pointer shadow-xl"
                >
                  <div className={`relative w-full ${SPAN_RATIO[item.span] || SPAN_RATIO.normal}`}>
                    ...
                    <div className="absolute top-3 left-3 z-10">
                      <span className="px-2.5 py-1 bg-black/70 backdrop-blur-md border border-white/10 text-[11px] font-bold uppercase tracking-wider text-[#DAC8B3]">
                        {item.category}
                      </span>
                    </div>
                    ...
                    <div className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300">
                      <Maximize2 className="w-4 h-4 text-[#C6A98A]" />
                    </div>
                    ...
                    <div className="absolute bottom-0 inset-x-0 p-4 sm:p-5 z-10 space-y-1 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                      <h3 className="font-fjalla text-lg sm:text-xl font-bold text-white uppercase tracking-wide line-clamp-1 group-hover:text-[#C6A98A] transition-colors duration-300">
                        {item.title}
                      </h3>
                      {item.caption && (
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="px-2.5 py-1 bg-[#C6A98A]/15 border border-[#C6A98A]/30 text-[#C6A98A] text-[11px] uppercase font-bold tracking-wider">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 rounded-full bg-zinc-900 text-zinc-300 hover:text-black hover:bg-[#C6A98A] transition-colors cursor-pointer"
          ...
          {playing && (
            <div className="max-w-7xl w-full mx-auto h-0.5 bg-white/10 mb-3 overflow-hidden">
              <div
                key={lightboxIndex}
                className="slide-progress h-full bg-[#C6A98A]"
          ...
          >
            <button
              onClick={() => step(-1)}
              className="absolute left-0 sm:left-2 z-20 p-3 rounded-full bg-black/60 hover:bg-[#C6A98A] hover:text-black text-white backdrop-blur-md transition-colors cursor-pointer shadow-2xl"
            ...
            <button
              onClick={() => step(1)}
              className="absolute right-0 sm:right-2 z-20 p-3 rounded-full bg-black/60 hover:bg-[#C6A98A] hover:text-black text-white backdrop-blur-md transition-colors cursor-pointer shadow-2xl"
